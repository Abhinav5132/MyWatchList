use crate::{
    backend::{login::LoginStruct, sign_up::AuthResponse, *},
    try_or,
};
use actix_web::{
...
pub async fn verify_entered_password(
    db: Data<Pool<Sqlite>>,
    req: HttpRequest,
    user: Json<LoginStruct>,
    ...
    let row = try_or!(
        sqlx::query("SELECT user_password FROM user WHERE id = ?")
            .bind(user_id)
            .fetch_one(db.as_ref())
            .await,
    ...
    let hash: &str = try_or!(
        row.try_get("user_password"),
        HttpResponse::InternalServerError().finish()
    );

    let verification = try_or!(
        verify_pwd(&user.password, hash),
