...
pub trait TokenVerifier: Sync + Send {
    ...
    fn get_userid_from_jwt<'b>(
        &'b self,
        token: &'b str,
    ) -> Pin<Box<dyn Future<Output = i64> + Send + 'b>>;
}
...
impl TokenVerifier for VerificationService {
    fn verify_token<'a>(
        ...
        Box::pin(async move {
            ...
            if let Ok(row) = sqlx::query("SELECT user_access_token FROM user WHERE id = ?")
                .bind(claims.sub)
...
impl TokenVerifier for MockVerificationService {
    ...
    fn get_userid_from_jwt<'b>(
        ...
        // always return 1 cuz test user should always be on 1
        Box::pin(async move {
            return 1;
        })
    }
